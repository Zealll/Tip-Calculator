...
const Result = props => {
    ...
    if(props.state){
        ...
        const bartenderTotal = bartender === 0 ? 0 : (total / 100) * 5 / bartender
        const busserTotal = busser === 0 ? 0 : (total / 100) * 15 / busser
        const serverTotal = server === 0 ? 0 : ((total - (bartenderTotal * bartender + busserTotal * busser)) / server)
